...
class DivideConquerSolver:
                ...
                for topt in cls_data["time_options"]:
                    ...
                    for ropt in cls_data.get("room_options", []):
                        ...
                        for cid2, (tidx2, rid2) in assignments.items():
                            ...
                            if self._times_conflict(tbits_opt, tbits2):
    ...
    @staticmethod
    def _times_conflict(bits1: tuple, bits2: tuple) -> bool:
        """Return True if two time_bits tuples conflict for SameAttendees / NotOverlap.

        ITC 2019 semantics: back-to-back classes (one ends exactly when the
        next starts) are considered conflicting — students cannot attend two
        consecutive classes with zero gap.  Uses inclusive-boundary overlap
        (<=) so that the boundary slot is treated as shared.
        """
        w1, d1, s1, l1 = bits1
        w2, d2, s2, l2 = bits2
        # Slot overlap — inclusive: back-to-back (s2 == s1+l1) is a conflict
