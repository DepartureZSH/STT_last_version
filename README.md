...
The solver can extract multiple different solutions from Gurobi's solution pool in a single run. Solutions are saved to a folder with sequential numbering.

```bash
# Generate 10 solutions (default)
python main.py --instance data/source/instances/agh-fal17.xml --technique hybrid

# Generate up to 100 solutions
python main.py --instance data/source/instances/agh-fal17.xml --technique hybrid --num-solutions 100

# With longer time limit for more solutions
python main.py --instance data/source/instances/agh-fal17.xml --technique hybrid --time-limit 3600 --num-solutions 100
```

Solutions are saved to `solutions/{instance-name}/solution{i}_{instance-name}.xml` where `i` ranges from 1 to the number of solutions found.

**Note**: The number of solutions generated depends on:
- Time limit (longer time = more solutions found)
- Instance difficulty
- `PoolSolutions` setting in `config.yaml` (default: 100)
